...
public class FiltersCursorAdapter extends ResourceCursorAdapter {
    ...
    private void reinit(Cursor cursor) {
        if (cursor != null && cursor.getCount() > 0) {
            mFilterTextColumnPosition = cursor.getColumnIndex(FeedData.FilterColumns.FILTER_TEXT);
            mIsAppliedToTitleColumnPosition = cursor.getColumnIndex(FeedData.FilterColumns.IS_APPLIED_TO_TITLE);
            mIsAcceptRulePosition = cursor.getColumnIndex(FeedData.FilterColumns.IS_ACCEPT_RULE);
        }
    }
